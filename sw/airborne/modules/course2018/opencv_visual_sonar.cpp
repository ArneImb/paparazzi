...
uint16_t func_square_height(uint16_t pos)
{
	...
	uint16_t square_height = (float)max_square_height-((float)max_square_height-(float)min_square_height)
			/((float)screen_height/2.)*((float)pos+(float)pitch_to_pix*theta);									//Linear function for square height
...
uint16_t pixels_to_go(Mat mask, uint8_t square_width = square_width, float threshold = square_th)
{
	...
	{
		uint16_t square_height = func_square_height(left_pos);													//Height of a pixel block
		...
		uint16_t right_pos = left_pos + square_width;															//Bottom position of pixel block
		uint16_t n_postives = number_positives_square(integral_mask, left_pos, right_pos, top_pos, bottom_pos);	//Number of positive pixels in a square
		if(n_postives < min_area)																				//Check if the positive pixels is below the threshold
		{
			break;
		}
	}
...
float pix_to_m(uint16_t pixels)
{
	...
	{
		float theta = stateGetNedToBodyEulers_f()->theta;														//Pitch angle in radians
		meters = 0.03328*((float)pixels+(float)pitch_to_pix*theta)+2.12956;										//Available distance
		meters *= aggression;																					//Scale with flight aggression
	}
	return meters;
}
...
int opencv_YCbCr_filter(char *img, int width, int height)
{
	...
	Mat M_RGB;																									//RedGreenBlue matrix
	Mat masked_RGB;																								//Masked RedGreenBlue matrix

	//Convert UYUV in paparazzi to YUV opencv
	cvtColor(M, M_RGB, CV_YUV2RGB_Y422);
	cvtColor(M_RGB, M, CV_RGB2YUV);
	Mat mask(M.size(), CV_8UC1, Scalar(0));																		//Initialize mask

	inRange(M,
			Scalar(color_lum_min, color_cb_min, color_cr_min),
			Scalar(color_lum_max, color_cb_max, color_cr_max), mask);											// Filter YCrCb values

	//Calculate distance to go
	pix_to_go = pixels_to_go(mask);																				//Pixels to go
	m_to_go = pix_to_m(pix_to_go);																				//Meters to go
	...
	for(disp_pos = 0; disp_pos<=pix_to_go; disp_pos += square_width)											//Loop to maximum pixel distance
	{
		...
		line(masked_RGB, Point(disp_pos,(height-square_height)/2),
				Point(disp_pos,(height+square_height)/2), Scalar(255,0,0),1);									//Draw line
	}

	cvtColor(masked_RGB, M, CV_RGB2YUV);																		//Convert back
	coloryuv_opencv_to_yuv422(M, img, width, height);															//Save in original image position
